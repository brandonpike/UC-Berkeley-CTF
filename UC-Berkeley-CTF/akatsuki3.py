...
class SafeAgent(CaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if agentState.numCarrying > 0:
      # Get closest border
      cB = min([self.getMazeDistance(agentState.getPosition(), b) for b in self.border]);
      features['closestBorder'] = cB;
      if(min(self.getRemainingMoves(gameState)) <= cB):
        features['timeExpiring'] = 1;
        print 'expiring';
  ...
  def getWeights(self, gameState, action):
    ...
    """
    return {
      'certainDeath': -1000000,
      'timeExpiring': -999500,
      'unguardedCapsules': -100000,
      'unguardedFood': -10000,
      'eatsEnemy': 10000,
      'eatsFood': 10000,
      'capturableFood': 1000,
      'closestEnemyDistance': -100,
      'closestFoodDistance': -50,
      'closestBorder': -1
    }
  ...
  def getRemainingMoves(self, gameState):
    # the index of the agent that moved last
    lastAgent = gameState.data._agentMoved
    # number of complete turns remaining in the game
    completeTurns = gameState.data.timeleft // gameState.getNumAgents()

    # put the number of moves each agent has left in a list
    numMoves = []
    for i in range(gameState.getNumAgents()):
      if i > lastAgent:
        numMoves.append(completeTurns + 1)
      else:
        numMoves.append(completeTurns)
